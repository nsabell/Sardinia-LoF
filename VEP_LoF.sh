...
for chrVCF in /users/nsabell/sardinia/vcfs/ss2120/*vcf.gz; do
	...
	cmd="export PERL5LIB=$PERL_LIB ; $VEP --offline -i $chrVCF -o $OUTDIR/$name.vepLoF.vcf --vcf --stats_text --cache --merged --dir_plugins LoF,human_ancestor_fa:$HUMAN_ANCESTOR_FA > $OUTDIR/$name.vepLoF.log 2>&1 &";
	echo $cmd;
